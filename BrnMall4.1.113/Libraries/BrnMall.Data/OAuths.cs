    ...
    public partial class OAuths
    {
        ...
        public static int GetUidByOpenIdAndServer(string openId, string server)
        {
            ...
            if (_usernosql != null)
            {
                ...
                if (uid < 1)
                {
                    uid = BrnMall.Core.BMAData.RDBS.GetUidByOpenIdAndServer(openId, server);
                    if (uid > 0)
        ...
        public static OAuthInfo GetOAuthUserByUid(int uid)
        {
            ...
            IDataReader reader = BrnMall.Core.BMAData.RDBS.GetOAuthUserByUid(uid);
            if (reader.Read())
            {
        ...
        public static List<OAuthInfo> GetOAuthUserList(string uidList)
        {
            ...
            IDataReader reader = BrnMall.Core.BMAData.RDBS.GetOAuthUserList(uidList);
            while (reader.Read())
            {
