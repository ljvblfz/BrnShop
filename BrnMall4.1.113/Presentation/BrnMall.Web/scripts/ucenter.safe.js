...
function verifyPassword() {
    ...
    $.post("/ucenter/verifypassword?act=" + act, { 'password': password, 'verifyCode': verifyCode }, function (data) {
...
function verifyMobile() {
    ...
    $.post("/ucenter/verifymobile?act=" + act, { 'moibleCode': moibleCode, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        if (result.state == "success") {
            window.location.href = result.content;
        }
...
function sendVerifyEmail() {
    ...
    $.post("/ucenter/sendverifyemail?act=" + act, { 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}
...
function updatePassword() {
    ...
    $.post("/ucenter/updatepassword?v=" + v, { 'password': password, 'confirmPwd': confirmPwd, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        if (result.state == "success") {
            window.location.href = result.content;
        }
...
function sendUpdateMobile() {
    ...
    $.post("/ucenter/sendupdatemobile?v=" + v, { 'mobile': mobile }, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}
...
function updateMobile() {
    ...
    $.post("/ucenter/updatemobile?v=" + v, { 'mobile': mobile, 'moibleCode': moibleCode, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        if (result.state == "success") {
            window.location.href = result.content;
        }
...
function sendUpdateEmail() {
    ...
    $.post("/ucenter/sendupdateemail?v=" + v, { 'email': email, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}