...
function login() {
    ...
    $.post("/mob/account/login", parms, loginResponse)
}
...
function register() {
    ...
    parms["verifyCode"] = verifyCode;
    $.post("/mob/account/register", parms, registerResponse)
}
...
function findPwd() {
    ...
    parms[shadowName] = accountName;
    parms["verifyCode"] = verifyCode;
    $.post("/mob/account/findpwd", parms, findPwdResponse)
}
...
function sendFindPwdMobile(uid) {
    $.get("/mob/account/sendfindpwdmobile?uid=" + uid, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}
...
function verifyFindPwdMobile(uid, mobileCode) {
    ...
    $.post("/mob/account/verifyfindpwdmobile?uid=" + uid, { 'mobileCode': mobileCode }, function (data) {
        var result = eval("(" + data + ")");
        if (result.state == "success") {
            window.location.href = result.content;
        }
...
function sendFindPwdEmail(uid) {
    $.get("/mob/account/sendfindpwdemail?uid=" + uid, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}
...
function resetPwd(v) {
    ...
    parms["password"] = password;
    parms["confirmPwd"] = confirmPwd;
    $.post("/mob/account/resetpwd?v=" + v, parms, resetPwdResponse)
}
