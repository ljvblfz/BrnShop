...
function verifyPassword() {
    ...
    $.post("/mob/ucenter/verifypassword?act=" + act, { 'password': password, 'verifyCode': verifyCode }, function (data) {
...
function sendVerifyMobile() {
    $.get("/mob/ucenter/sendverifymobile", function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}
...
function verifyMobile() {
    ...
    $.post("/mob/ucenter/verifymobile?act=" + act, { 'moibleCode': moibleCode, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        if (result.state == "success") {
            window.location.href = result.content;
        }
...
function sendVerifyEmail() {
    ...
    $.post("/mob/ucenter/sendverifyemail?act=" + act, { 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}
...
function updatePassword() {
    ...
    $.post("/mob/ucenter/updatepassword?v=" + v, { 'password': password, 'confirmPwd': confirmPwd, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        if (result.state == "success") {
            window.location.href = result.content;
        }
...
function sendUpdateMobile() {
    ...
    $.post("/mob/ucenter/sendupdatemobile?v=" + v, { 'mobile': mobile }, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}
...
function updateMobile() {
    ...
    $.post("/mob/ucenter/updatemobile?v=" + v, { 'mobile': mobile, 'moibleCode': moibleCode, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        if (result.state == "success") {
            window.location.href = result.content;
        }
...
function sendUpdateEmail() {
    ...
    $.post("/mob/ucenter/sendupdateemail?v=" + v, { 'email': email, 'verifyCode': verifyCode }, function (data) {
        var result = eval("(" + data + ")");
        alert(result.content)
    })
}