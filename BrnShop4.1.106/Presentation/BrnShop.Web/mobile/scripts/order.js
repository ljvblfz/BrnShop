...
function getValidCouponList() {
    $.get("/mob/order/getvalidcouponlist?selectedCartItemKeyList=" + $("#selectedCartItemKeyList").val(), getValidCouponListResponse);
}
...
function getValidCouponListResponse(data) {
    ...
    if (result.state == "success") {
        if (result.content.length < 1) {
            $("#validCouponList").html("<div class=\"allCell\">此订单暂无可用的优惠券</div>");
        }
        else {
            var itemList = "";
            for (var i = 0; i < result.content.length; i++) {
...
function submitOrder() {
    ...
    if (!verifySubmitOrder(saId, shipName)) {
        return;
    }

    $.post("/mob/order/submitorder",
            { 'selectedCartItemKeyList': selectedCartItemKeyList, 'saId': saId, 'payMode': payMode, 'shipName': shipName, 'payCreditCount': payCreditCount, 'couponIdList': couponIdList, 'fullCut': fullCut },
            submitOrderResponse)
}

//验证提交订单
function verifySubmitOrder(saId, shipName) {
    if (saId < 1) {
        alert("请填写收货人信息");
        return false;
    }
