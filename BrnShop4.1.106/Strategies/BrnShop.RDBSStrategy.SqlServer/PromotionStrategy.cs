...
namespace BrnShop.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetBuySendPromotionList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} {2} FROM [{1}buysendpromotions] ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.BUYSEND_PROMOTIONS);

                else
                    commandText = string.Format("SELECT TOP {0} {3} FROM [{1}buysendpromotions] WHERE {2} ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                TableFields.BUYSEND_PROMOTIONS);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}buysendpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.BUYSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                    ...
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}buysendpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.BUYSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
        ...
        public DataTable AdminGetSuitPromotionList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [pmid],[starttime1],[endtime1],[state],[name] FROM [{1}suitpromotions] ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT TOP {0} [pmid],[starttime1],[endtime1],[state],[name] FROM [{1}suitpromotions] WHERE {2} ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}suitpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.SUIT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize);
                    ...
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}suitpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.SUIT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
        ...
        public DataTable AdminGetFullSendPromotionList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [pmid],[starttime],[endtime],[state],[name],[limitmoney],[addmoney] FROM [{1}fullsendpromotions] ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT TOP {0} [pmid],[starttime],[endtime],[state],[name],[limitmoney],[addmoney] FROM [{1}fullsendpromotions] WHERE {2} ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullsendpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.FULLSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize);
                    ...
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullsendpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.FULLSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
        ...
        public DataTable AdminGetFullCutPromotionList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} {2} FROM [{1}fullcutpromotions] ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.FULLCUT_PROMOTIONS);

                else
                    commandText = string.Format("SELECT TOP {0} {3} FROM [{1}fullcutpromotions] WHERE {2} ORDER BY [pmid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                TableFields.FULLCUT_PROMOTIONS);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullcutpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.FULLCUT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize);
                    ...
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullcutpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}",
                                                TableFields.FULLCUT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
