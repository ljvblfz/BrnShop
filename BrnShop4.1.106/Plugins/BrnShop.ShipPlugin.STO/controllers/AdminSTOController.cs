...
namespace BrnShop.Web.Admin.Controllers
{
    ...
    /// 后台申通快递控制器类
    /// </summary>
    public class AdminSTOController : BaseAdminController
    {
        /// <summary>
        /// 配送规则列表
        /// </summary>
        ...
        public ActionResult AddShipRule()
        {
            ...
            Load(-1, -1, -1);
            return View("~/plugins/BrnShop.ShipPlugin.STO/views/adminsto/addshiprule.cshtml", model);
        }
        ...
        public ActionResult EditShipRule(ShipRuleModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                AddAdminOperateLog("编辑申通快递配送规则");
                return PromptView(Url.Action("config", "plugin", new { configController = "AdminSTO", configAction = "ShipRuleList" }), "配送规则编辑成功");
            }

            return PromptView(Url.Action("config", "plugin", new { configController = "AdminSTO", configAction = "EditShipRule", shipRuleName = shipRuleInfo.Name }), "内容有误，请重写填写");
        }
