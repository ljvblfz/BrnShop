...
namespace BrnShop.Web.Admin.Controllers
{
    ...
    /// 后台顺丰快递控制器类
    /// </summary>
    public class AdminSFController : BaseAdminController
    {
        /// <summary>
        /// 配送规则列表
        /// </summary>
        ...
        public ActionResult ShipRuleList()
        {
            ...
            return View("~/plugins/BrnShop.ShipPlugin.SF/views/adminsf/shiprulelist.cshtml", model);
        }
        ...
        public ActionResult EditShipRule(ShipRuleModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                AddAdminOperateLog("编辑顺丰快递配送规则");
                return PromptView(Url.Action("config", "plugin", new { configController = "AdminSF", configAction = "ShipRuleList" }), "配送规则编辑成功");
            }

            return PromptView(Url.Action("config", "plugin", new { configController = "AdminSF", configAction = "EditShipRule", shipRuleName = shipRuleInfo.Name }), "内容有误，请重写填写");
        }
        ...
        public ActionResult DelShipRule(string shipRuleName = "")
        {
            ...
            PluginUtils.SaveShipRuleList(shipRuleList);
            AddAdminOperateLog("删除顺丰快递配送规则");
            return PromptView(Url.Action("config", "plugin", new { configController = "AdminSF", configAction = "ShipRuleList" }), "配送规则删除成功");
        }
    }
}
