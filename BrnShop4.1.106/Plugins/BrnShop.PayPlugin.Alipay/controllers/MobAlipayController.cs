...
namespace BrnShop.Web.Mobile.Controllers
{
    ...
    public class AlipayController : Controller
    {
        ...
        public ActionResult Pay()
        {
            ...
            string notify_url = string.Format("http://{0}/mob/alipay/notify", BSPConfig.ShopConfig.SiteUrl);
            //页面跳转同步通知页面路径，需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/
            string return_url = string.Format("http://{0}/mob/alipay/return", BSPConfig.ShopConfig.SiteUrl);

            //商户订单号
            string out_trade_no = orderInfo.Oid.ToString() + Randoms.CreateRandomValue(10, false);
            //商户网站订单系统中唯一订单号，必填
            ...
            //付款金额
            string total_fee = orderInfo.SurplusMoney.ToString();
            //必填

            //商品展示地址
            string show_url = string.Format("http://{0}/images/alipay.jgp", BSPConfig.ShopConfig.SiteUrl);
